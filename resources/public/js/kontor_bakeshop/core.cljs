...
(defmethod mutate 'cart/add
  ...
  {:action
   (fn []
     (swap! state update :cart (fn [c] (vec (distinct (conj c [:treat/by-id id])))))
     (swap! state update-in [:treat/by-id id] update :count inc)
     )})

(defmethod mutate 'cart/empty
  [{:keys [state]} _ _]
  {:action
   (fn []
     (map (fn [id] (swap! state update-in [:treat/by-id id] update :count 0)) [1 2 3 4])
...
(defn subtotal [count price bulkPricing]
            (if (or (nil? bulkPricing) (> (get bulkPricing :amount) count))
              (* count price)
              (+
                (* (quot count (get bulkPricing :amount))
                   (get bulkPricing :totalPrice))
                (* (rem count (get bulkPricing :amount))
                   price)))
            )
...
(defui CartItem
  ...
  (render [this]
    (let [{:keys [id count name price bulkPricing] :as props} (om/props this)]
      (dom/div
        #js {:react-key (str "cart-item-" name) :id (str "cart-item-" id) :className "cart-item"}
        (dom/div #js {:className "cart-item-name"} name)
        (dom/div #js {:className "cart-item-count"} (dom/span nil (str count "x")))
             ...
             (dom/div #js {:id "cart"} [
                                          ...
                                          (when (not (empty? cart))
                                            [(dom/div #js {:id "cart-total"}
                                             ...
                                             (dom/div #js{:id "cart-actions"}
                                                      [(dom/button #js{:id "cart-empty" :onClick (fn [e] (om/transact! this '[(cart/empty)]))} "empty cart")
                                                       (dom/button #js{:id "cart-submit" :onClick (fn [e] (om/transact! this '[(cart/empty)]))} "purchase!")])])])
             ))))
